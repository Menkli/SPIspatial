#' Builds a raster brick from a netCDF list
#'
...
br <- raster::brick(nrows=y, ncols=x, xmn=min(lon), xmx=max(lon), ymn=min(lat), ymx=max(lat), nl=z, crs=4326)

# Here, the empty brick is populated with the precipitation data
setValues(br, prec_array)
}
